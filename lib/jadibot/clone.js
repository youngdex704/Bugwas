...
const jadibot = async (afgan, m, from) => {
if (Object.keys(client).includes(from)) {
    return afgan.sendMessage(from, { text: "Kamu sudah jadibot sebelumnya!" }, {quoted: m})
  }
const { state, saveCreds } = await useMultiFileAuthState(`./data/afgan-clone/${m.sender.split("@")[0]}`)
try {
...
if (usePairingCode && !client[from].user && !client[from].authState.creds.registered) {
 ...
 contextInfo: {
   ...
   await afgan.relayMessage(m.chat, msg.message, {
...
require('../../baseafgan')(client[from], m, chatUpdate, mek, store)
}
} catch (err) {
console.log(err)
}
...
connectToWhatsApp();
  ...
  afgan.sendMessage(from, { text: `Bot Clone Terhubung...` }, {quoted: m});
  await sleep(3000)
...
async function stopjadibot(afgan, m, from) {
  if (!Object.keys(client).includes(from)) {
    return afgan.sendMessage(from, { text: `Kamu tidak ada di list jadibot!` }, {quoted: m})
  }
  try {
    client[from].end("Stop")
  } catch {}
  delete client[from]
  rimraf.sync(`./data/afgan-clone/${from.split("@")[0]}`)
}

async function listjadibot(afgan, m) {
  ...
  return afgan.sendMessage(from, { text: text.trim(), mentions, }, {quoted: m})
}
